...
#define BSP_BOOT_ADDRESS 0x00000000
...
void bsp_set_scu_boot_addr(phys_addr_t start_addr, phys_addr_t jump_addr)
{
	void __iomem *virt;
...
int bsp_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
	...
	bsp_set_scu_boot_addr(BSP_BOOT_ADDRESS, jumpaddr);
	bsp_set_cpu(cpu, true);
