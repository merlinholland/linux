 ...
 *           system memory. Reduce printk log lantecy
...
static char time_stamp_str[TIME_STAMP_LEN] = {0};
...
char *iprec_tm(void)
{
	...
	tv = ktime_to_timeval(ktime);

	ret = sprintf_s(time_stamp_str, TIME_STAMP_LEN, "[%04ld.%03ld%03ld]",
			tv.tv_sec % SEC_FOUR_DIGIT,
			tv.tv_usec / USEC_TO_MSEC,
			tv.tv_usec % USEC_TO_MSEC);
	if (ret < 0)
...
static void iprec_show(struct seq_file *s)
{
	...
	if (rec_ovwr == 0) {
		...
		for (i = 0; i < rec_line; i++) {
			seq_printf(s, " %04d %s\n", (i + rec_lines - rec_line + 1), rec_pool[i]);
		}
	}
...
static const struct file_operations iprec_dump_ops = {
	.owner = THIS_MODULE,
	.open = iprec_proc_dump_open,
	.read = seq_read,
};
	...
	const char __user *buffer, size_t count, loff_t *pos)
{
	...
	// rec_lines update, so remalloc new memory then free old memory;
...
static const struct file_operations iprec_line_ops = {
	.owner = THIS_MODULE,
	.open = iprec_proc_line_open,
	.read = seq_read,
	.write = iprec_proc_line_write,
};
...
static const struct file_operations iprec_lock_ops = {
	.owner = THIS_MODULE,
	.open = iprec_proc_lock_open,
	.read = seq_read,
	.write = iprec_proc_lock_write,
};
...
static const struct file_operations iprec_delay_ops = {
	.owner = THIS_MODULE,
	.open = iprec_proc_delay_open,
	.read = seq_read,
	.write = iprec_proc_delay_write,
};

static struct proc_dir_entry *iprec_create_proc_node(const char *name,
	struct proc_dir_entry *proc_entry, const struct file_operations *fileops)
{
...
static int __init iprec_proc_init(void)
{
	...
	if (iprec_init == 0) {
		...
		if (iprec_create_proc_node(IPREC_DUMP, iprec_proc, &iprec_dump_ops) == NULL)
			goto remove_entry;

		if (iprec_create_proc_node(IPREC_LINE, iprec_proc, &iprec_line_ops) == NULL)
			goto remove_dump;

		if (iprec_create_proc_node(IPREC_LOCK, iprec_proc, &iprec_lock_ops) == NULL)
			goto remove_line;

		if (iprec_create_proc_node(IPREC_DLAY, iprec_proc, &iprec_delay_ops) == NULL)
			goto remove_lock;
