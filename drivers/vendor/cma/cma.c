...
int __init declare_heap_memory(void)
{
	...
	for (i = 0; i < num_zones; i++) {
		ret = dma_declare_contiguous(&bsp_zone[i].pdev,
					     bsp_zone[i].nbytes, bsp_zone[i].phys_start, 0);
		if (ret)
		...
		bsp_zone[i].phys_start = 
			cma_get_base(bsp_zone[i].pdev.cma_area);
		bsp_zone[i].nbytes = cma_get_size(bsp_zone[i].pdev.cma_area);
	}

	return ret;
}
EXPORT_SYMBOL(declare_heap_memory);
