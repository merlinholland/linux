...
/*************************************************************/
// #define DUMP_UVC_DATA_FROM_MPP
// FOR DUMP UVC DATA FROM MPP
...
#define FILE_NAME "/run/uvc_dump_file"

static struct file *dump_file;
struct uvc_pack_save {
	struct list_head list;
	uint64_t     addr;
	unsigned int len;
	spinlock_t lock;
};
static struct uvc_pack_save g_uvc_pack_save;
struct work_struct save_file_worker;
static unsigned long long current_offset;

static void pack_save_to_save_list(struct uvc_pack_save *ptr)
{
	unsigned long flags;

	spin_lock_irqsave(&g_uvc_pack_save.lock, flags);
	list_add_tail(&ptr->list, &g_uvc_pack_save.list);
	spin_unlock_irqrestore(&g_uvc_pack_save.lock, flags);
}

static void save_file_open(void)
{
	mm_segment_t oldfs;

	oldfs = get_fs();
	set_fs(get_ds());
	dump_file = filp_open(FILE_NAME, O_CREAT | O_RDWR | O_TRUNC, S_IRWXU | S_IRWXG | S_IRWXO);
	set_fs(oldfs);
	if (IS_ERR(dump_file)) {
		dump_file = NULL;
		return;
...
void save_file_close(void)
{
	struct uvc_pack_save *p;
	unsigned long flags;

	if (dump_file == NULL)
		return;

	cancel_work_sync(&save_file_worker);
	...
	spin_lock_irqsave(&g_uvc_pack_save.lock, flags);
	while (!list_empty(&g_uvc_pack_save.list)) {
		p = list_first_entry(&g_uvc_pack_save.list, struct uvc_pack_save, list);
		list_del(&p->list);
		kfree(p);
	}
	spin_unlock_irqrestore(&g_uvc_pack_save.lock, flags);

	INIT_LIST_HEAD(&g_uvc_pack_save.list);
}

int save_file_write(unsigned long long offset, unsigned char *data, unsigned int size)
{
	mm_segment_t oldfs;
	int ret;
	...
	ret = vfs_write(dump_file, data, size, &offset);
...
void save_file_thread(struct work_struct *work)
{
	struct uvc_pack_save *p;
	unsigned long flags;
	...
	while (!list_empty(&g_uvc_pack_save.list)) {
		...
		list_del(&p->list);
		spin_unlock_irqrestore(&g_uvc_pack_save.lock, flags);
		save_file_write(current_offset, (void *)p->addr, p->len);
		current_offset += p->len;
		kfree(p);
		spin_lock_irqsave(&g_uvc_pack_save.lock, flags);
	}
	spin_unlock_irqrestore(&g_uvc_pack_save.lock, flags);
		...
		struct uvc_pack *pack, bool is_frame_end, bool free)
{
	...
	memcpy(ptr->pack, pack, sizeof(struct uvc_pack));
...
#ifdef DUMP_UVC_DATA_FROM_MPP
int uvc_recv_pack_save(uint64_t addr, unsigned int len)
{
	struct uvc_pack_save *p;
	p = (struct uvc_pack_save *)kmalloc(sizeof(struct uvc_pack_save), GFP_KERNEL);
	if (p != NULL) {
		p->addr = addr;
		p->len = len;
		pack_save_to_save_list(p);
	}
...
int uvc_recv_pack(struct uvc_pack *pack)
{
	...
	if (pack->buf_vir_addr == 0 || pack->pack_vir_addr == 0 || pack->pack_len == 0) {
		...
		printk(KERN_EMERG"###%s###: buf_vir_addr:0x%llx size:%d pack_vir_addr:0x%llx len:%d end:%u \
			callback:0x%px\n", __func__, pack->buf_vir_addr, pack->buf_size, pack->pack_vir_addr, pack->pack_len,
			pack->is_frame_end, pack->callback_func);
...
#ifdef DUMP_UVC_DATA_FROM_MPP
		uvc_recv_pack_save(p->addr, p->len);
		uvc_recv_pack_save(q->addr, q->len);
#endif
	} else {
		p->len = pack->pack_len;
		p->addr = pack->pack_vir_addr;
...
#ifdef DUMP_UVC_DATA_FROM_MPP
	if (likely(dump_file != NULL)) {
		schedule_work(&save_file_worker);
	}
#endif
...
#ifdef DUMP_UVC_DATA_FROM_MPP
	INIT_LIST_HEAD(&g_uvc_pack_save.list);
	spin_lock_init(&g_uvc_pack_save.lock);
	INIT_WORK(&save_file_worker, save_file_thread);
#endif
#if IS_ENABLED(CONFIG_MPP_TO_GADGET_UVC)
