...
EXPORT_SYMBOL(bsp_sata_mem_dump);

void bsp_sata_phys_mem_dump(unsigned int addr, unsigned int size)
{
	bsp_sata_mem_dump(phys_to_virt(addr), size);
}
EXPORT_SYMBOL(bsp_sata_phys_mem_dump);
...
void bsp_ahci_reg_dump(void)
{
	...
	regbase = CONFIG_BSP_SATA_IOBASE;
	...
	regbase = CONFIG_BSP_SATA_IOBASE + 0x0100;
...
EXPORT_SYMBOL(bsp_ahci_reg_dump);
...
void bsp_ahci_rx_fis_dump(struct ata_link *link, int pmp_port_num)
{
	...
	bsp_sata_phys_mem_dump((unsigned int)(pp->rx_fis_dma),
				AHCI_RX_FIS_SZ * pmp_port_num);
	pr_debug("\n");
}
EXPORT_SYMBOL_GPL(bsp_ahci_rx_fis_dump);
...
EXPORT_SYMBOL_GPL(bsp_ata_taskfile_dump);

static void __bsp_ahci_st_md(void __iomem *addr)
{
...
void bsp_ahci_st_dump(const void __iomem *port_base)
{
	...
	__bsp_ahci_st_md(phys_to_virt(tmp));
	__bsp_ahci_st_md(phys_to_virt(tmp + 0x100));
	__bsp_ahci_st_md(phys_to_virt(tmp + 0x200));
	__bsp_ahci_st_md(phys_to_virt(tmp + 0x300));
}
EXPORT_SYMBOL_GPL(bsp_ahci_st_dump);
