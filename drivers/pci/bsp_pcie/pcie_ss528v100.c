...
struct pcie_iatu iatu1_table[] = {
	{
		.viewport = 0, /* iAtu Vierport Register */
		.region_ctrl_1 = 0x00000004, /* Region Control 1 Register */
		.region_ctrl_2 = 0x90000000, /* Region Control 2 Register */
		.lbar = PCIE_EP1_CONF_BASE + (1 << 20), /* Lower Base Address Register */
		.ubar = 0x0, /* Upper Base Address Register */
		.lar = PCIE_EP1_CONF_BASE + (2 << 20) - 1, /* Limit Address Register */
		.ltar = 0x01000000, /* Lower Target Address Register */
		.utar = 0x00000000, /* Upper Target Address Register */
	},
	{
		...
		.region_ctrl_1 = 0x00000005, /* Region Control 1 Register */
		.region_ctrl_2 = 0x90000000, /* Region Control 2 Register */
		.lbar = PCIE_EP1_CONF_BASE + (2 << 20), /* Lower Base Address Register */
		.ubar = 0x0, /* Upper Base Address Register */
		.lar = PCIE_EP1_CONF_BASE + (__128MB__ - 1), /* Limit Address Register */
		.ltar = 0x02000000, /* Lower Target Address Register */
		.utar = 0x00000000, /* Upper Target Address Register */
	},
};
				   ...
				   struct pci_sys_data *sys)
{
	...
	if (info->controller == 1) {
		ptable = iatu1_table;
		table_size = ARRAY_SIZE(iatu1_table);

		ctl1_lbar_offset = (unsigned int)(info->root_bus_nr) << 20;
...
static int __arch_pcie_info_setup(struct pcie_info *info, int *controllers_nr)
{
	...
	if ((nr == 1) && pcie_controllers_nr == 0) {
		pr_err("Pcie controler 0 is disabled by comphy config!\n");
		return -EINVAL;
	}
	...
	info_tmp->conf_base_addr = (uintptr_t)ioremap_nocache(property.pcie_dbi_base, __8KB__);
