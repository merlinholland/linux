...
#define BSP_COMB_PHY1_TEST_CTRL	0x1c8
...
static void  bsp_sata0_rx0_reset(void *crg_base)
{
...
static void bsp_sata0_rx0_unreset(void *crg_base)
{
	unsigned int reg_val;
...
static void bsp_sata1_rx1_reset(void *crg_base)
{
	unsigned int reg_val;
...
static void bsp_sata1_rx1_unreset(void *crg_base)
{
	unsigned int reg_val;
...
void bsp_sata_reset_rxtx_assert(unsigned int port_no)
{
	...
	comb_phy_crg_base = ioremap((BSP_CRG_REG_BASE + COMB_PHY1_PORT_A_REG), SATA_CRG_MAP_SIZE);
	...
	case CASE_NUM0:
		...
		reg_val = readl(comb_phy_crg_base);
		reg_val |= COMB_PHY_REST;
		writel(reg_val, comb_phy_crg_base);
		break;
	case CASE_NUM1:
		reg_val = readl(crg_base + 0x40);
		reg_val |= (SATA_CTRL_RX_RST | SATA_CTRL_SATA_RST);
		...
		reg_val = readl(comb_phy_crg_base + 0x20);
		reg_val |= COMB_PHY_REST;
		writel(reg_val, comb_phy_crg_base + 0x20);
		break;
	default:
		break;
	}
...
void bsp_sata_reset_rxtx_deassert(unsigned int port_no)
{
	...
	crg_base = ioremap((BSP_CRG_REG_BASE + SATA_CLK_RST_CTRL_1_REG), SATA_CRG_MAP_SIZE);
	if (!crg_base) {
		pr_err("ioremap sata reset register base failed! func:%s, line:%d\n", __func__,
		       __LINE__);
		iounmap(comb_phy_crg_base);
...
static void bsp_sata_reset(void)
{
	...
	switch (ports_num) {
	case CASE_NUM2:
		bsp_sata0_rx0_reset(crg_base);
		bsp_sata1_rx1_reset(crg_base);
		break;
	default:
		break;
	}
...
static void bsp_sata_unreset(void)
{
	...
	switch (ports_num) {
	case CASE_NUM2:
		bsp_sata0_rx0_unreset(crg_base);
		bsp_sata1_rx1_unreset(crg_base);
		break;
	default:
		break;
	}
...
static void bsp_sata_port1_reset(void *crg_base)
{
	unsigned int reg_val;
...
static void bsp_sata_port1_unreset(void *crg_base)
{
	unsigned int reg_val;
...
static void bsp_sata_port0_reset(void *crg_base)
{
	unsigned int reg_val;
...
static void bsp_sata_port0_unreset(void *crg_base)
{
	unsigned int reg_val;
...
static void bsp_sata_phy_reset(void)
{
	...
	case CASE_NUM2:
		/* COMBPHY1 PORT B */
		bsp_sata_port1_reset(comb_phy_crg_base);

		/* COMBPHY1 PORT A */
		bsp_sata_port0_reset(comb_phy_crg_base);
		break;
	default:
		break;
	}
...
static void bsp_sata_phy_unreset(void)
{
	...
	case CASE_NUM2:
		/* COMBPHY1 PORT B */
		bsp_sata_port1_unreset(comb_phy_crg_base);

		/* COMBPHY1 PORT A */
		bsp_sata_port0_unreset(comb_phy_crg_base);
		break;
	default:
		break;
	}
...
static void bsp_sata_port0_clk_enable(void *crg_base)
{
	unsigned int reg_val;
...
static void bsp_sata_port1_clk_enable(void *crg_base)
{
	unsigned int reg_val;
...
static void bsp_sata_clk_enable(void)
{
	...
	switch (ports_num) {
	case CASE_NUM2:
		bsp_sata_port0_clk_enable(crg_base);
		bsp_sata_port1_clk_enable(crg_base);
		break;
	default:
		break;
	}
	...
	iounmap(crg_base);
	udelay(300); /* delay 300 us */
}

static void bsp_sata_clk_disable(void)
{
	printk("----------bsp_sata_clk_diable--------\n");
}
...
static void bsp_sata_port1_clk_sel(void *crg_base)
{
	unsigned int reg_val;
...
static void bsp_sata_port0_clk_sel(void *crg_base)
{
	unsigned int reg_val;
...
static void bsp_sata_phy_clk_sel(void)
{
	...
	case CASE_NUM2:
		/* COMBPHY1 PORT B */
		bsp_sata_port1_clk_sel(comb_phy_crg_base);

		/* COMBPHY1 PORT A */
		bsp_sata_port0_clk_sel(comb_phy_crg_base);
		break;
	default:
		break;
	}
...
void bsp_sata_set_fifoth(void *mmio)
{
	...
	for (port_idx = 0; port_idx < ports_num; port_idx++) {
		...
		writel(FIFOTH_VALUE2, (mmio + 0x100 + port_idx * 0x80 + PORT_FIFOTH2));
	}
}
EXPORT_SYMBOL(bsp_sata_set_fifoth);

static void bsp_sata_port1_config(void *misc_base)
{
	/* cfg COMBPHY1 PORT B */
...
static void bsp_sata_port0_config(void *misc_base)
{
...
static void bsp_sata_port_cfg(void)
{
	...
	case CASE_NUM2:
		/* cfg COMBPHY1 PORT B */
		bsp_sata_port1_config(misc_base);

		/* cfg COMBPHY1 PORT A */
		bsp_sata_port0_config(misc_base);
		break;
	default:
		break;
	}
...
#define IOCONFIG1_REG_MAP_SIZE	0x200
static void bsp_sata_set_led(void)
{
	...
	for (port_idx = 0; port_idx < ports_num; port_idx++) {
		if (port_idx == 0)
			writel(0x1201, reg_addr + 0xFC);
		else
			writel(0x1201, reg_addr + 0x100);
	}
...
static void bsp_sata_phy_config(void *mmio, int phy_mode)
{
	...
	for (port_idx = 0; port_idx < ports_num; port_idx++) {
		writel(PX_TX_DEEMPH, (mmio + 0x100 + port_idx * 0x80 + BSP_SATA_PORT_PHYCTL2));
		writel(PX_TX_AMPLITUDE, (mmio + 0x100 + port_idx * 0x80 +
					 BSP_SATA_PORT_PHYCTL1));
