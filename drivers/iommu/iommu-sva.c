...
static void iommu_notifier_release_ex(struct mmu_notifier *mn, struct mm_struct *mm)
{
	svm_smmu_clk_live_enter();
	iommu_notifier_release(mn, mm);
...
static struct mmu_notifier_ops iommu_mmu_notifier = {
	.release		= iommu_notifier_release_ex,
...
void iommu_sva_flush_iotlb_single(struct mm_struct *mm)
{
	int i;
	struct io_mm *io_mm = NULL;
	struct iommu_bond *bond = NULL;
	...
	idr_for_each_entry(&iommu_pasid_idr, io_mm, i) {
		if (io_mm->mm == mm && io_mm_get_locked(io_mm)) {
			list_for_each_entry(bond, &io_mm->devices, mm_head) {
				struct iommu_domain *domain = bond->domain;
				domain->ops->flush_iotlb_single(domain, io_mm);
			}
...
EXPORT_SYMBOL_GPL(iommu_sva_flush_iotlb_single);

const char *iommu_sva_get_smmu_device_name(struct mm_struct *mm)
{
	int i;
	struct io_mm *io_mm = NULL;
	struct iommu_bond *bond = NULL;
...
EXPORT_SYMBOL_GPL(iommu_sva_get_smmu_device_name);
