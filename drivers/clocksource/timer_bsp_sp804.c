/*
 * Copyright (c) 2019 Shenshu Technologies Co., Ltd.
 *
...
{
	struct bsp_sp804_clockevent_device *bsp_clkevt = to_clkevt(clkevt);

	writel(0, bsp_clkevt->base + TIMER_CTRL);
					     ...
					     struct clock_event_device *clkevt)
{
	...
	writel(TIMER_CTRL_32BIT, bsp_clkevt->base + TIMER_CTRL);

	writel(next, bsp_clkevt->base + TIMER_LOAD);
	writel(next, bsp_clkevt->base + TIMER_LOAD);
	...
	ctrl = TIMER_CTRL_32BIT |
	       ...
	       TIMER_CTRL_ONESHOT |
	       TIMER_CTRL_ENABLE;
	writel(ctrl, bsp_clkevt->base + TIMER_CTRL);
...
static int sp804_clockevent_set_periodic(struct clock_event_device *clkevt)
{
	...
	writel(bsp_clkevt->reload, bsp_clkevt->base + TIMER_LOAD);
	writel(bsp_clkevt->reload, bsp_clkevt->base + TIMER_LOAD);
...
static irqreturn_t bsp_sp804_clockevent_timer_interrupt(int irq, void *dev_id)
{
	...
	/* clear the interrupt */
	writel(1, bsp_clkevt->base + TIMER_INTCLR);
...
static int bsp_sp804_clockevent_setup(struct bsp_sp804_clockevent_device *bsp_clkevt)
{
	struct clock_event_device *clkevt = &bsp_clkevt->clkevt;

	writel(0, bsp_clkevt->base + TIMER_CTRL);

	BUG_ON(setup_irq(clkevt->irq, &bsp_clkevt->action));

	irq_force_affinity(clkevt->irq, clkevt->cpumask);

	clockevents_config_and_register(clkevt, bsp_clkevt->rate, 0xf,
		0x7fffffff);

	return 0;
}

static void bsp_sp804_clockevent_stop(struct bsp_sp804_clockevent_device *bsp_clkevt)
{
	struct clock_event_device *clkevt = &bsp_clkevt->clkevt;
	...
	remove_irq(clkevt->irq, &bsp_clkevt->action);
...
static void __init clockevent_init(struct bsp_sp804_clockevent_device *bsp_clkevt,
				   ...
				   unsigned long rate, unsigned long reload)
{
	...
	bsp_clkevt->base = base;
	bsp_clkevt->rate = rate;
	bsp_clkevt->reload = reload;
	if (snprintf_s(bsp_clkevt->name, DEV_NAME_LEN, DEV_NAME_LEN, "clockevent %d", cpu) < 0)
		printk("snprintf_s failed! func:%s, line: %d\n", __func__, __LINE__);

	clkevt = &bsp_clkevt->clkevt;

	clkevt->name = bsp_clkevt->name;
	...
	action = &bsp_clkevt->action;

	action->name = bsp_clkevt->name;
	action->dev_id = bsp_clkevt;
	action->irq = irq;
