...
static unsigned char fmc100_read_byte(struct mtd_info *mtd)
{
	...
	if (host->cmd_op.l_cmd == NAND_CMD_READID) {
		value = fmc_readb((void __iomem *)(chip->IO_ADDR_R + host->offset));
		host->offset++;
...
static unsigned short fmc100_read_word(struct mtd_info *mtd)
{
	...
	host->offset += WORD_READ_OFFSET_ADD_LENGTH;
	return fmc_readw(host->buffer + host->column +
		host->offset - WORD_READ_START_OFFSET);
}
				   ...
				   int *is_cache_invalid)
{
	...
	case NAND_CMD_READID:
		...
		if (memset_s((u_char *)(chip->IO_ADDR_R), FMC_MEM_LEN,
				0, MAX_NAND_ID_LEN)) {
...
static void fmc100_chip_init(struct nand_chip *chip)
{
	...
	if (memset_s((char *)chip->IO_ADDR_R, FMC_MEM_LEN, 0xff, host->dma_len)) {
		printk("%s %d:memset_s failed\n", __func__, __LINE__);
		return;
	}
