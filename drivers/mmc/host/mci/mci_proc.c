...
static int unstuff_bits(const int *resp, int start, int size)
{
...
static int mci_host_seq_printout(struct seq_file *s, const struct mci_host *host,
...
static void mci_cinfo_seq_printout(struct seq_file *s, const struct card_info *c_info,
...
{
	unsigned int index_mci, clock, clock_scale;
	...
	for (index_mci = 0; index_mci < MCI_SLOT_NUM; index_mci++) {
		...
		if (mci_host_seq_printout(s, host, index_mci))
			continue;
		...
		if (CARD_CONNECT != c_info->card_connect) {
			...
			mci_cinfo_seq_printout(s, c_info, uhs_bus_speed_mode);
			clock = host->hclk;
