...
static void bsp_set_drv_str(struct sdhci_host *host, unsigned int offset,
...
#endif
static void sdhci_bsp_enhanced_strobe(struct mmc_host *mmc,
    struct mmc_ios *ios)
{
	u16 ctrl;
...
int bsp_support_runtime_pm(struct sdhci_host *host)
{
	...
	/* only eMMC device support runtime_pm */
	return (priv->devid == 0) ? 1 : 0;
}
